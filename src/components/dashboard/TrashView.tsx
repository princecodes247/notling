      ...
      <div className="max-w-5xl mx-auto flex flex-col gap-8">
          ...
          <div className="flex items-center gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono font-semibold px-2 py-0.5 rounded-full bg-stone-100 text-stone-600 border border-stone-200">
          ...
          {trashPages.length > 0 && (
            <div className="flex items-center gap-2.5 shrink-0">
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg border border-stone-200/90 hover:bg-stone-100 text-stone-800 text-xs font-semibold transition-colors cursor-pointer shadow-2xs"
              >
        ...
        {trashPages.length > 0 && (
          <div className="relative">
            <div className="flex items-center gap-2.5 px-3.5 py-2 rounded-lg bg-stone-50 border border-stone-200/80 focus-within:border-stone-400 focus-within:bg-white transition-colors">
        ...
        <div className="flex flex-col gap-3">
          {trashPages.length === 0 ? (
            <div className="py-20 border border-dashed border-stone-200 rounded-xl flex flex-col items-center justify-center text-center p-8 gap-3 text-stone-400 bg-stone-50/40">
              ...
              <div className="flex flex-col gap-1">
                <span className="text-sm font-semibold text-stone-700">Trash is empty</span>
                <span className="text-xs text-stone-400">Pages you delete in your workspace will appear here.</span>
              </div>
            </div>
          ) : filteredTrash.length === 0 ? (
            <div className="py-12 border border-dashed border-stone-200 rounded-xl text-center text-xs text-stone-400 p-6">
              No trashed pages match "{filterQuery}"
            </div>
          ) : (
            <div className="flex flex-col divide-y divide-stone-100 rounded-xl border border-stone-200/90 bg-white overflow-hidden shadow-2xs">
              {filteredTrash.map((item) => (
                <div
                  key={item.id}
                  className="p-4 hover:bg-stone-50/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-colors group"
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    <span className="text-xl shrink-0 p-1.5 bg-stone-100 rounded-lg">{item.icon || '📄'}</span>
                  ...
                  <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-stone-100 hover:bg-stone-200 text-stone-800 text-xs font-semibold transition-colors cursor-pointer border border-stone-200/80 shadow-2xs"
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-rose-50 text-stone-400 hover:text-rose-700 text-xs font-medium transition-colors cursor-pointer"
